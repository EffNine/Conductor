...
// TestP312CacheKeyProviderIdentityIsolation locks the P3.12 cache identity
...
func TestP312CacheKeyProviderIdentityIsolation(t *testing.T) {
...
// TestP312CacheKeyProviderPrefixRoutesIsolate verifies the real-world
...
// different providers.
func TestP312CacheKeyProviderPrefixRoutesIsolate(t *testing.T) {
	h := testCacheHandler(t)
...
// TestP312CacheKeyFallbackRouteChangesProvider verifies that when a primary
// route falls back to a second provider for the same resolved model, the
// cache entries stay isolated.
func TestP312CacheKeyFallbackRouteChangesProvider(t *testing.T) {
	h := testCacheHandler(t)
	req := &apitypes.ChatCompletionRequest{
		Model:    "m",
		Mode:     "auto",
...
// TestP312CacheKeyAliasToDifferentProvidersIsolate verifies two aliases that
// resolve to the SAME provider+model share a key, while aliases resolving to
// DIFFERENT providers do not.
func TestP312CacheKeyAliasToDifferentProvidersIsolate(t *testing.T) {
	h := testCacheHandler(t)
...
// TestP312CacheKeyCanonicalModeNormalization verifies the mode dimension of
...
func TestP312CacheKeyCanonicalModeNormalization(t *testing.T) {
	h := testCacheHandler(t)
	req := &apitypes.ChatCompletionRequest{
		Model:    "m",
		Mode:     "auto",
...
// TestP312CacheKeyModeAndProviderOrthogonal verifies mode and provider
// dimensions are independent: changing either one changes the key, and
// identical (mode, provider) pairs agree.
func TestP312CacheKeyModeAndProviderOrthogonal(t *testing.T) {
	h := testCacheHandler(t)
	req := &apitypes.ChatCompletionRequest{
		Model:    "m",
		Mode:     "auto",
