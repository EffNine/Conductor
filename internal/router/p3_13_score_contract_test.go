...
// TestP313CompositeScoreArithmetic verifies the exact composite formula
// against the trace for all eight active modes.
func TestP313CompositeScoreArithmetic(t *testing.T) {
...
// TestP313ModeWeightsSumToOne: every active mode's normalized weights are a
// valid convex combination.
func TestP313ModeWeightsSumToOne(t *testing.T) {
	for _, mode := range []string{"auto", "coding", "reasoning", "vision", "fast", "planning", "agentic", "long_horizon"} {
		w := modeWeights(t, mode)
