...
// TestP311ModelTelemetryOverridesProvider verifies telemetry precedence:
...
func TestP311ModelTelemetryOverridesProvider(t *testing.T) {
...
// TestP311PartialModelTelemetryFallsBackToProvider verifies the P3.12
...
func TestP311PartialModelTelemetryFallsBackToProvider(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestP311TelemetryProviderFallbackWhenModelAbsent verifies provider-level
// telemetry IS used when no model-level entry exists at all.
func TestP311TelemetryProviderFallbackWhenModelAbsent(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
