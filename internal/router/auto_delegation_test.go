...
// TestAutoDelegatesToRouterClassifier: the legacy automode classifier is a
// thin wrapper over router.ClassifyRequest — no independent classification
// logic exists.
func TestAutoDelegatesToRouterClassifier(t *testing.T) {
	cases := []struct {
		text string
		want string
	}{
...
// TestAutoIsDefaultNotClassifier: Mode="auto" with coding-looking text
...
func TestAutoIsDefaultNotClassifier(t *testing.T) {
...
// TestAutoNoReclassificationAcrossRequests: the trace event bus captures
// each decision's own trace; sequential auto decisions do not leak state.
func TestAutoNoReclassificationAcrossRequests(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
...
// TestAutoEliteClassificationFails: the classifier can resolve to the
...
func TestAutoEliteClassificationFails(t *testing.T) {
