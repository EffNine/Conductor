...
// TestVisionCapabilityUnknownStillEligibleWithoutImage verifies mode=vision with NO
...
func TestVisionCapabilityUnknownStillEligibleWithoutImage(t *testing.T) {
...
// TestTextMentionOfImageNotMultimodal verifies that merely TALKING about
...
func TestTextMentionOfImageNotMultimodal(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "vision-p", supportsAll: true, vision: true, latencyMs: 150, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "text-p", supportsAll: true, vision: false, latencyMs: 50, healthState: runtime.StateHealthy},
	)
...
// TestUppercaseModeNormalized verifies the public mode API normalizes
...
func TestUppercaseModeNormalized(t *testing.T) {
...
// TestWhitespaceModeNormalized verifies whitespace-padded mode strings are
// trimmed to the canonical mode (P3.12 normalization contract).
func TestWhitespaceModeNormalized(t *testing.T) {
...
// TestPipelineAcceptsNormalizedMode verifies the pipeline accepts
// non-canonical spellings and routes with the canonical mode.
func TestPipelineAcceptsNormalizedMode(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestExactPublicModeStringsAccepted verifies every documented public mode
// string parses to the canonical Mode and no more.
func TestExactPublicModeStringsAccepted(t *testing.T) {
