...
// TestP312NormalizeModeContract locks the canonical normalization function:
// lowercase + trim; empty input stays empty (distinct from "auto").
func TestP312NormalizeModeContract(t *testing.T) {
	cases := []struct {
		in   string
		want string
	}{
...
// TestP312ParseModeAcceptsNormalizedSpellings verifies ParseMode accepts
// non-canonical spellings and maps them to the canonical Mode.
func TestP312ParseModeAcceptsNormalizedSpellings(t *testing.T) {
	cases := []struct {
		in   string
		want router.Mode
	}{
...
// TestP312ParseModeStillRejectsUnknownValues verifies normalization does NOT
// make unknown values valid: non-mode strings and internal modes still error.
func TestP312ParseModeStillRejectsUnknownValues(t *testing.T) {
...
// TestP312NormalizedModeStillRejectsInvalidPipelineInput verifies the
// pipeline surfaces an invalid-mode error for truly unknown mode strings
// even after normalization.
func TestP312NormalizedModeStillRejectsInvalidPipelineInput(t *testing.T) {
...
// TestP312RequestedModePreservedRaw verifies the raw user-supplied mode string
// is preserved in DecisionContext.RequestedMode (auditability) while the
// resolved ModeProfile uses the canonical mode.
func TestP312RequestedModePreservedRaw(t *testing.T) {
