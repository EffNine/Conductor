...
// TestContextFilterProportional verifies the long_horizon context filter
...
func TestContextFilterProportional(t *testing.T) {
...
// TestMaxTokensOverrideAffectsRequirement verifies req.MaxTokens
...
func TestMaxTokensOverrideAffectsRequirement(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestThinkingBudgetAffectsRequirement verifies thinking_budget tokens
// contribute to the context requirement at the pipeline level.
func TestThinkingBudgetAffectsRequirement(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestResponseFormatDoesNotConsumeContext verifies structured-output
// requests are not penalized by the context filter (response_format is not
// part of the token estimate).
func TestResponseFormatDoesNotConsumeContext(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "ctx8192", supportsAll: true, maxContext: 8192, latencyMs: 50, healthState: runtime.StateHealthy},
	)
	setHealth(t, store, "ctx8192", runtime.StateHealthy, 50)
...
// TestSlightlyBelowRejectedExactlyEqualAccepted verifies the boundary
// semantics at the pipeline level using max_tokens to control the estimate.
func TestSlightlyBelowRejectedExactlyEqualAccepted(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
