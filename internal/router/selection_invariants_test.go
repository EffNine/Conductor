...
// TestAddingWorseCandidateCannotChangeWinner verifies the selection
// invariant that adding a worse candidate never changes the winner.
func TestAddingWorseCandidateCannotChangeWinner(t *testing.T) {
...
// TestRemovingWinnerSelectsBestRemaining verifies that when the winner is
// removed from the explicit candidate set, the best remaining candidate is
// selected.
func TestRemovingWinnerSelectsBestRemaining(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestReorderingExplicitCandidates verifies explicit candidate order only
// affects the outcome when scores are exactly tied.
func TestReorderingExplicitCandidates(t *testing.T) {
...
// TestModeProfileMutationIsolation verifies mode profiles are copied per
// decision: mutating one decision's profile must not affect another.
func TestModeProfileMutationIsolation(t *testing.T) {
...
// TestHardRejectedNeverSelected verifies the invariant that a
// hard-rejected candidate can never be selected, even when it is the only
// candidate.
func TestHardRejectedNeverSelected(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
