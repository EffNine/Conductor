...
// TestP311NormalizeSumsToOne verifies weight normalization invariants.
func TestP311NormalizeSumsToOne(t *testing.T) {
...
// TestP311NormalizeZeroFallsBackToEqual verifies degenerate raw weights fall
// back to equal weights instead of panicking or producing NaN.
func TestP311NormalizeZeroFallsBackToEqual(t *testing.T) {
...
// TestP311WeightsInfluenceScores verifies each weight genuinely changes the
// composite score in the intended direction.
func TestP311WeightsInfluenceScores(t *testing.T) {
...
// TestP311SingleWeightChangeFlipsWinner verifies the Phase 4 acceptance case:
// changing ONE weight (latency) flips the winner for identical candidates with
// a health-vs-latency tradeoff.
func TestP311SingleWeightChangeFlipsWinner(t *testing.T) {
	scorer := router.NewScorer(router.RawWeights{Health: 40, Latency: 25, Cost: 15, Capability: 20})
...
// TestP311BonusesAffectScores verifies capability bonuses change scores in the
// intended direction and are bounded (never dominate the weighted base).
func TestP311BonusesAffectScores(t *testing.T) {
	scorer := router.NewScorer(router.RawWeights{Health: 40, Latency: 25, Cost: 15, Capability: 20})
...
// TestP311BonusCannotBeatLargeBaseGap verifies bonuses cannot overturn a large
// legitimate base-score difference.
func TestP311BonusCannotBeatLargeBaseGap(t *testing.T) {
	scorer := router.NewScorer(router.RawWeights{Health: 40, Latency: 25, Cost: 15, Capability: 20})
...
// TestP311ModeProfilesHaveDistinctNormalizedWeights verifies mode profiles do
...
// context requirement).
func TestP311ModeProfilesHaveDistinctNormalizedWeights(t *testing.T) {
