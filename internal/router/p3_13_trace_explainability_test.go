...
// TestP313TraceCarriesCandidateScores: the trace exposes the same candidate
// scores as the decision result (the Phase 10 gap fix: previously the trace
// carried only the Winner and no scores).
func TestP313TraceCarriesCandidateScores(t *testing.T) {
...
// TestP313TraceCarriesRejections: rejected candidates appear in the trace with
...
func TestP313TraceCarriesRejections(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
...
// TestP313TraceLifecycleEvents: the trace contains the full decision lifecycle
// (decision.started, stage events, intent.resolved with mode metadata).
func TestP313TraceLifecycleEvents(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
