...
// TestVisionHardFilterRegression verifies a non-vision provider is hard-
...
func TestVisionHardFilterRegression(t *testing.T) {
...
// TestVisionHardFilterExplicitRoutes verifies the vision hard filter also
// applies on the pre-resolved-route path (selectFromRoutesWithMode).
func TestVisionHardFilterExplicitRoutes(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestFPTieBreakRegression verifies that scores differing only by
...
func TestFPTieBreakRegression(t *testing.T) {
...
// TestHardFilterPrecedence verifies hard rejections (planning/agentic
...
func TestHardFilterPrecedence(t *testing.T) {
	cases := []struct {
		name      string
		mode      string
...
// TestBreakerHardFilterThroughPipeline verifies an open circuit breaker
// rejects a candidate before soft scoring on the DecisionPipeline path.
func TestBreakerHardFilterThroughPipeline(t *testing.T) {
...
// TestHardFilterCannotBeOutweighedByBonus verifies a candidate rejected by
// a hard filter cannot win through capability bonuses or telemetry: the
// rejected flag is checked before score comparison.
func TestHardFilterCannotBeOutweighedByBonus(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
