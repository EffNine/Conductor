...
// TestContextBoundaryExactAndOneAbove verifies the boundary is
// INCLUSIVE at equality and exclusive one token above:
// required 8196 == MaxContext 8196 -> eligible; 8195 -> rejected.
func TestContextBoundaryExactAndOneAbove(t *testing.T) {
...
// TestContextUnknownMaxContextAlwaysEligible verifies MaxContext == 0
// (unknown) never triggers the hard filter.
func TestContextUnknownMaxContextAlwaysEligible(t *testing.T) {
...
// TestContextDefaultOutputBudget verifies absent MaxTokens uses the
// DefaultMaxTokens (4096) output budget: required = 4 + 4096 = 4100.
func TestContextDefaultOutputBudget(t *testing.T) {
...
// TestContextEstimateCountsMultimodal verifies image content adds 5
...
func TestContextEstimateCountsMultimodal(t *testing.T) {
...
// TestContextEstimateCountsTools verifies tool definitions add to the
// input estimate: 1 tool "t" adds 8 + (1+3)/4 = 9 tokens;
// required = 4 + 9 + 8192 = 8205.
func TestContextEstimateCountsTools(t *testing.T) {
	req := ctxReq("long_horizon", 8192)
...
// TestContextFilterOnlyLongHorizonAndAgentic verifies NO other mode
// enforces the context budget, even for an absurdly large request.
func TestContextFilterOnlyLongHorizonAndAgentic(t *testing.T) {
