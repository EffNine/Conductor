...
// TestP314SuccessfulDecisionTraceContract (matrix A + M): for all eight
...
func TestP314SuccessfulDecisionTraceContract(t *testing.T) {
...
// TestP314TraceDecisionIdentity (matrix C + D): the selected candidate in the
...
func TestP314TraceDecisionIdentity(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 100, healthState: runtime.StateHealthy, costPerUnit: 0.0002, maxContext: 128000},
...
// TestP314CandidateScoreComponentIdentity (matrix C): for every mode and every
...
func TestP314CandidateScoreComponentIdentity(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 100, healthState: runtime.StateHealthy, costPerUnit: 0.0002, maxContext: 128000},
		&calibStubProvider{name: "zzz", supportsAll: true, vision: false, reasoning: false, toolCalling: false, structured: false, latencyMs: 2000, healthState: runtime.StateDegraded, costPerUnit: 0.0009, maxContext: 128000},
	)
...
// TestP314CustomCostCeilingConsistency: with a non-default engine cost ceiling,
...
func TestP314CustomCostCeilingConsistency(t *testing.T) {
...
// TestP314PlanningAgenticTelemetryPrefIdentity: planning/agentic candidates
...
func TestP314PlanningAgenticTelemetryPrefIdentity(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 100, healthState: runtime.StateHealthy, costPerUnit: 0.0002, maxContext: 128000},
...
// TestP314RejectionReasonIdentity (matrix G + P/Q/R/S): every hard rejection
// path is traceable as candidate -> reason. UNKNOWN (MaxContext=0) is never
// converted into a rejection.
func TestP314RejectionReasonIdentity(t *testing.T) {
	cases := []struct {
		name         string
...
// TestP314UnknownContextIsNotRejection: UNKNOWN (MaxContext=0) must remain
// distinct from FALSE — an unknown-context candidate is never hard-rejected
// and the trace must not claim a rejection for it.
func TestP314UnknownContextIsNotRejection(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
...
// TestP314AllCandidatesRejectedTrace (matrix B + Phase 14): every candidate
// rejected -> trace explains without logs: no winner, rejection reasons on
// every candidate.
func TestP314AllCandidatesRejectedTrace(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
...
// TestP314CircuitBreakerOpenRejection: an open breaker produces a traceable
// "circuit breaker open" rejection and no winner.
func TestP314CircuitBreakerOpenRejection(t *testing.T) {
	reg := provider.NewRegistry()
	store := runtime.NewRuntimeStore(nil)
...
// TestP314InactiveModeFailureTrace: a decision that fails at the intent stage
...
func TestP314InactiveModeFailureTrace(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
...
// TestP314InvalidModeFailureTrace: an invalid explicit mode publishes a trace
// with the raw requested mode preserved.
func TestP314InvalidModeFailureTrace(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, latencyMs: 50, healthState: runtime.StateHealthy, costPerUnit: 0.0001, maxContext: 128000},
	)
	setHealth(t, store, "aaa", runtime.StateHealthy, 50)
...
// TestP314ExplicitRouteTrace (matrix N): a pre-resolved candidate set produces
// a trace whose winner is within the set, in the given order.
func TestP314ExplicitRouteTrace(t *testing.T) {
	reg := provider.NewRegistry()
	store := runtime.NewRuntimeStore(nil)
...
// TestP314AutoModeTraceIdentity (matrix O): explicit auto resolves to the
// default profile with source "explicit" and default policy metadata.
func TestP314AutoModeTraceIdentity(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, latencyMs: 50, healthState: runtime.StateHealthy, costPerUnit: 0.0001, maxContext: 128000},
	)
	setHealth(t, store, "aaa", runtime.StateHealthy, 50)
...
// TestP314NoSecondSnapshot (matrix J + Phase 15): the pipeline acquires
// exactly ONE RuntimeSnapshot per decision, and the trace's RuntimeHash is
// computed from that exact snapshot.
func TestP314NoSecondSnapshot(t *testing.T) {
...
// TestP314NoProviderApiCalls (matrix K): trace construction performs no
...
func TestP314NoProviderApiCalls(t *testing.T) {
	reg := provider.NewRegistry()
	store := runtime.NewRuntimeStore(nil)
...
// TestP314ConcurrentTraceConstruction (matrix L): concurrent decisions each
...
func TestP314ConcurrentTraceConstruction(t *testing.T) {
	reg := provider.NewRegistry()
	store := runtime.NewRuntimeStore(nil)
	aaa := &calibStubProvider{name: "aaa", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 100, healthState: runtime.StateHealthy, costPerUnit: 0.0002, maxContext: 128000}
	zzz := &calibStubProvider{name: "zzz", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 300, healthState: runtime.StateHealthy, costPerUnit: 0.0009, maxContext: 128000}
...
// TestP314TraceSchemaVersionDocumented (matrix I): the schema version is a
// deliberate, documented constant distinct from RuntimeHash.
func TestP314TraceSchemaVersionDocumented(t *testing.T) {
...
// TestP314TraceJSONSerialization (matrix H): DecisionTrace round-trips through
// JSON deterministically, handles nil/empty/rejection-only/large cases, and
// never leaks request content.
func TestP314TraceJSONSerialization(t *testing.T) {
...
// TestP314TraceNeverLeaksRequestContent: serialized traces must not contain
// prompt text, image URLs, or request payload material.
func TestP314TraceNeverLeaksRequestContent(t *testing.T) {
	pipeline, store, rec := setupCalibPipelineTraced(t,
		&calibStubProvider{name: "aaa", supportsAll: true, vision: true, reasoning: true, toolCalling: true, structured: true, latencyMs: 100, healthState: runtime.StateHealthy, costPerUnit: 0.0002, maxContext: 128000},
	)
	setHealth(t, store, "aaa", runtime.StateHealthy, 100)
