...
// TestUnhealthySoleCandidateEligibleAcrossModes verifies an unhealthy
// provider is never hard-rejected: as the sole candidate it is selected in
// every mode.
func TestUnhealthySoleCandidateEligibleAcrossModes(t *testing.T) {
...
// TestUnhealthyLosesToHealthyBySoftMargin verifies unhealthy is a soft
...
func TestUnhealthyLosesToHealthyBySoftMargin(t *testing.T) {
	for _, mode := range p312AllPublicModes {
		pipeline, store, _ := setupCalibPipeline(t,
...
// TestCircuitOpenHardRejectsAcrossModes verifies an open breaker hard-
// rejects the candidate in every mode, even when it is the sole candidate.
func TestCircuitOpenHardRejectsAcrossModes(t *testing.T) {
	for _, mode := range p312AllPublicModes {
...
// TestCircuitOpenBeatsEverySoftAdvantage verifies a circuit-open candidate
// loses to a healthy competitor even when it dominates on latency, cost, and
// capabilities.
func TestCircuitOpenBeatsEverySoftAdvantage(t *testing.T) {
	reg := provider.NewRegistry()
	store := runtime.NewRuntimeStore(nil)
