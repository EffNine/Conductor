...
// TestP312PlanningPrefersExecutionReliableProvider verifies planning uses the
// telemetry preference: aaa's measured-good model history outranks zzz's
// cheaper pricing and larger context.
func TestP312PlanningPrefersExecutionReliableProvider(t *testing.T) {
...
// TestP312LongHorizonPrefersCostAndContext verifies long_horizon ignores the
// telemetry preference: zzz wins on cost (both contexts satisfy the small
// request), despite its poor model telemetry.
func TestP312LongHorizonPrefersCostAndContext(t *testing.T) {
	pipeline, _ := setupPlanHorizonPipeline(t)
...
// TestP312LongHorizonHardContextRejectsSmallContext verifies the context hard
...
func TestP312LongHorizonHardContextRejectsSmallContext(t *testing.T) {
	pipeline, _ := setupPlanHorizonPipeline(t)
...
// TestP312PlanningHasNoContextHardFilter verifies planning does NOT enforce a
// context budget: the same oversized request leaves aaa eligible and aaa wins
// on telemetry.
func TestP312PlanningHasNoContextHardFilter(t *testing.T) {
	pipeline, _ := setupPlanHorizonPipeline(t)
...
// TestP312PlanningAndAgenticRequireReasoningTooling locks the shared hard
...
func TestP312PlanningAndAgenticRequireReasoningTooling(t *testing.T) {
