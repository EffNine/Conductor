...
// TestMeasuredPoorModelDoesNotFallBackToProvider locks the key rule: a
...
func TestMeasuredPoorModelDoesNotFallBackToProvider(t *testing.T) {
...
// TestMeasuredGoodModelBeatsMeasuredGoodProvider locks the reverse side:
...
func TestMeasuredGoodModelBeatsMeasuredGoodProvider(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestInsufficientModelNeutralWhenProviderInsufficient verifies that when
// BOTH levels are INSUFFICIENT the signal stays neutral and the deterministic
// tie-break decides (aaa).
func TestInsufficientModelNeutralWhenProviderInsufficient(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestUnknownModelNeutralWhenProviderUnknown verifies that when neither
// level has ANY telemetry the signal is neutral (tie-break decides).
func TestUnknownModelNeutralWhenProviderUnknown(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestToolSignalFallsBackIndependently verifies per-signal independence:
...
func TestToolSignalFallsBackIndependently(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestProviderToolBonusAppliesWhenModelToolUnknown locks the positive
...
// at model level.
func TestProviderToolBonusAppliesWhenModelToolUnknown(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestRetrySignalMeasuredPoorModelNoFallback verifies the retry signal
...
func TestRetrySignalMeasuredPoorModelNoFallback(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestRetrySignalFallsBackWhenModelInsufficient verifies the retry signal
...
func TestRetrySignalFallsBackWhenModelInsufficient(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
...
// TestTelemetryPreferenceAppliesInPlanningMode verifies the same
...
func TestTelemetryPreferenceAppliesInPlanningMode(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
		&calibStubProvider{name: "aaa", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
		&calibStubProvider{name: "zzz", supportsAll: true, reasoning: true, toolCalling: true, latencyMs: 100, healthState: runtime.StateHealthy},
	)
