...
// TestP311FastVsDefaultCanDiffer verifies that Fast mode can select a different
...
// fast provider.
func TestP311FastVsDefaultCanDiffer(t *testing.T) {
...
// TestP311CodingVsDefaultCanDiffer verifies Coding mode can select a different
...
func TestP311CodingVsDefaultCanDiffer(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311ReasoningVsDefaultCanDiffer verifies Reasoning mode can select a
// different winner than Default when reasoning capability is the tradeoff.
func TestP311ReasoningVsDefaultCanDiffer(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311PlanningVsReasoningDiffer verifies Planning hard-rejects candidates
// lacking tool calling while Reasoning merely soft-scores them.
func TestP311PlanningVsReasoningDiffer(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311AgenticVsPlanningCanDiffer verifies Agentic and Planning can select
...
func TestP311AgenticVsPlanningCanDiffer(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311LongHorizonVsDefaultDiffer verifies Long Horizon rejects a candidate
// whose known MaxContext is below the request requirement while Default accepts it.
func TestP311LongHorizonVsDefaultDiffer(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311LongHorizonContextThreshold verifies Phase 2's canonical case:
// a request requiring >32k tokens rejects the 32k candidate and accepts 128k.
func TestP311LongHorizonContextThreshold(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestP311AgenticTelemetryPreferred verifies Agentic prefers good-telemetry
// candidates when telemetry has sufficient observations (Phase 2 canonical case).
func TestP311AgenticTelemetryPreferred(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
