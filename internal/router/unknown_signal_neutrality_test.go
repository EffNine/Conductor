...
// TestUnknownLatencyNeutral verifies a candidate without recorded latency
// gets the documented neutral latency score (0.5), not 0 or an error.
func TestUnknownLatencyNeutral(t *testing.T) {
...
// TestUnknownCostNeutral verifies a candidate without pricing info gets
// the documented neutral cost score (0.5) and is still routable.
func TestUnknownCostNeutral(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestUnknownProviderStateNeutral verifies a provider absent from the
// snapshot gets the neutral health score and cannot be silently favored.
func TestUnknownProviderStateNeutral(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestUnhealthyScoresLowButNotRejected verifies the unhealthy state is a
// soft penalty (0.1), not a hard rejection: a single unhealthy candidate still
// gets selected rather than failing the request.
func TestUnhealthyScoresLowButNotRejected(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestUnknownMaxContextEligible verifies unknown MaxContext keeps a
// candidate eligible for long_horizon (no false hard rejection).
func TestUnknownMaxContextEligible(t *testing.T) {
	pipeline, store, _ := setupCalibPipeline(t,
...
// TestUnknownTelemetryNeutralInPlanning verifies planning telemetry is
...
func TestUnknownTelemetryNeutralInPlanning(t *testing.T) {
